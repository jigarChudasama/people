...
function shuffleIds() {
  const ids = Array.from({ length: PEOPLE_COUNT }, (_, i) => i + 1);
...
const CURSOR_COLORS = CLOTHES_COLORS.filter(
  (c) => !["#F8FAFC", "#FDE68A", "#FFEDD5"].includes(c)
);
...
/** Recolor the cursor body; sticker border stays white, outline stays dark. */
function colorizeCursorSvg(svg, color) {
  if (/fill="#2563EB"/i.test(svg)) {
    return svg.replace(/fill="#2563EB"/i, `fill="${color}"`);
  }
  let whiteIndex = 0;
  return svg.replace(/fill="white"/gi, (match) => {
    whiteIndex += 1;
    return whiteIndex === 2 ? `fill="${color}"` : match;
  });
}

function cursorDataUrl(svg) {
  return `url("data:image/svg+xml,${encodeURIComponent(svg)}")`;
}

function colorizePeople(html, seed) {
...
function getColoredHtml(peopleId, template, seed) {
  const key = `${peopleId}:${seed}`;
  let html = COLORIZE_CACHE.get(key);
  if (!html) {
    html = colorizePeople(template, seed);
    COLORIZE_CACHE.set(key, html);
  }
  return html;
}

function resolveTemplate(svgs, peopleId) {
  if (svgs[peopleId]) return { id: peopleId, template: svgs[peopleId] };
  const keys = Object.keys(svgs);
  if (keys.length === 0) return null;
  const id = Number(keys[peopleId % keys.length]);
...
export default function PeoplePop() {
  ...
  useEffect(() => {
    ...
    async function applyColoredCursors() {
      try {
        const [arrowRes, handRes] = await Promise.all([
          fetch("/arrow-cursor.svg"),
          fetch("/hand-cursor.svg"),
        ]);
        if (!arrowRes.ok || !handRes.ok) return;

        const [arrowSvg, handSvg] = await Promise.all([
          arrowRes.text(),
          handRes.text(),
        ]);
        if (cancelled) return;
        ...
        root.style.setProperty(
          "--cursor-arrow",
          cursorDataUrl(colorizeCursorSvg(arrowSvg, color))
        );
        root.style.setProperty(
          "--cursor-hand",
          cursorDataUrl(colorizeCursorSvg(handSvg, color))
        );
      } catch {
        // Keep default SVG cursors from CSS fallbacks.
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadFromBundle() {
      const res = await fetch("/people-bundle.json");
    ...
    async function loadIndividually() {
      ...
      async function worker() {
        while (cursor <= PEOPLE_COUNT) {
          const id = cursor;
          cursor += 1;
          const res = await fetch(`/people/people-${id}.svg`);
          const text = await res.text();
          next[id] = prepareSvg(text);
          done += 1;
          if (!cancelled) {
            setLoadProgress(Math.round((done / PEOPLE_COUNT) * 100));
            if (done === 16 || done === 40 || done === PEOPLE_COUNT) {
              setSvgs({ ...next });
            }
          }
        }
    ...
    async function loadPeople() {
      try {
        ...
        if (cancelled) return;
        const normalized = {};
        for (let i = 1; i <= PEOPLE_COUNT; i += 1) {
  ...
  useEffect(() => {
    ...
    const blinkTimer = setInterval(() => {
      for (const el of pickSlots(2 + Math.floor(3 * Math.random()))) {
        el.classList.add("people-blink");
        setTimeout(
          () => el.classList.remove("people-blink"),
          300 + 200 * Math.random()
        );
      }
    }, 900);
  ...
  const slotMeta = useMemo(() => {
    ...
    for (let row = 0; row < GRID; row += 1) {
      for (let col = 0; col < GRID; col += 1) {
        ...
        const peopleId = SHUFFLED_IDS[index % SHUFFLED_IDS.length];
  ...
  const visibleSlots = useMemo(() => {
    ...
    for (const slot of slotMeta) {
      if (
        slot.left + PEOPLE_W < minX ||
        slot.left > maxX ||
        slot.top + PEOPLE_H < minY ||
        slot.top > maxY
      ) {
        continue;
      }
      const resolved = resolveTemplate(svgs, slot.peopleId);
  ...
  if (!svgs) {
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center gap-3 bg-people-bg">
    ...
    <div className="viewport" ref={viewportRef}>
      ...
      >
        {visibleSlots.map(({ key, row, left, top, html, factors }) => (
          <div
            key={key}
            className={`people-slot${!loaded ? " people-slot--enter" : ""}`}
      ...
      <div className="control-bar" role="toolbar" aria-label="Crowd controls">
